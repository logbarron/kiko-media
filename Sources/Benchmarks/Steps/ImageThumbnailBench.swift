...
func benchmarkImageThumbnails(corpus: [MediaFile]) throws {
    BenchmarkRuntimeRenderer.printSubsectionTitle("Image Thumbnail Generation (512px)")
    ...
    let tmpDir = makeTempDir("img-thumb")
    defer { cleanup(tmpDir) }
    for file in images {
        var durations: [Double] = []
        for i in 0..<iterations {
            let out = "\(tmpDir)/thumb-\(file.name)-\(i).jpg"
            let d = try measure {
